              ...
              .then(response => {
                ...
                filteredSchedules.forEach(schedule => {
                  ...
                  const baseSchedule = {
                    ...
                    color: '#DD385A' // Use standard color for all approved schedules
                  };
      ...
      try {
        ...
        if (displayedSchedules) {
          ...
          if (Array.isArray(parsedDisplayedSchedules) && parsedDisplayedSchedules.length > 0) {
            ...
            this.schedules = parsedDisplayedSchedules.filter(s => s.status === 'approved');
        ...
        if (viewerSchedules) {
          ...
          if (Array.isArray(parsedViewerSchedules) && parsedViewerSchedules.length > 0) {
            // Only use approved schedules
            this.schedules = parsedViewerSchedules.filter(s => s.status === 'approved');
      ...
      try {
        ...
        systemSettingsAPI.getCurrentDisplaySemester()
          .then(semesterResponse => {
            ...
            // Load all schedules for the System Admin view
            return scheduleAPI.getAll()
              .then(response => {
                const allSchedules = response.data || [];
                
                // Filter schedules by the current display semester and only approved status
                const filteredSchedules = allSchedules.filter(schedule => 
                  schedule.semester_id === currentDisplaySemesterId && 
                  schedule.status === 'approved' // Only show approved schedules on dashboard
                );
                
                // Process the schedules to match the frontend format
                const processedSchedules = [];
                
                filteredSchedules.forEach(schedule => {
                  // Create the base schedule object - all are approved now
                  const baseSchedule = {
                    id: schedule.id,
                    semester: schedule.semester_name,
                    section: schedule.section,
                    ...
                    title: `${schedule.course_code} (${schedule.class_type})`,
                    details: `${schedule.course_name}\n${schedule.section}\n${schedule.instructor_name}`,
                    color: '#DD385A' // Use the standard color for all approved schedules
                  };
