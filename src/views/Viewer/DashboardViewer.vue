...
export default {
  ...
  methods: {
              ...
              .then(response => {
                ...
                filteredSchedules.forEach(schedule => {
                  ...
                  const baseSchedule = {
                    ...
                    color: '#DD385A' // Pink color for approved schedules
                  };
    ...
    // Add a fallback method to load from localStorage if API fails
        ...
        if (viewerSchedules) {
          try {
            ...
            if (Array.isArray(parsedViewerSchedules) && parsedViewerSchedules.length > 0) {
              ...
              let approvedViewerSchedules = parsedViewerSchedules.filter(schedule => schedule.status === 'approved');
              
              // If we have a current semester ID, filter by it
              if (currentDisplaySemesterId && approvedViewerSchedules.some(s => s.semester_id)) {
                approvedViewerSchedules = approvedViewerSchedules.filter(s => 
                  s.semester_id === currentDisplaySemesterId
                );
                console.log(`Filtered viewer schedules to semester ID ${currentDisplaySemesterId}:`, approvedViewerSchedules.length);
              }
          ...
          .then(semesterResponse => {
            ...
            return scheduleAPI.getByStatus('approved')
              .then(response => {
                ...
                // Only update the schedules array if something has actually changed
