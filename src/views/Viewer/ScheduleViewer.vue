        ...
        <div class="header">
          <div class="user-info">
            <div class="user-info-flex">
              ...
              <div v-if="user.classification === 'Student'" class="info-row">
                <span class="label">Year & Section:</span>
                <span class="value">{{ user.year_section || 'Not specified' }}</span>
              </div>
            </div>
...
export default {
  name: 'ScheduleViewer',
  components: {
    DashBoardSidebar: DashBoardSideBarViewer,
    DashBoardTopbar
  },
  data() {
    return {
...
methods: {
  ...
  async loadSchedulesWithoutFlicker() {
    try {
      ...
      // For students, add section filtering
      if (this.user.classification === 'Student' && this.user.year_section) {
        params.section = this.user.year_section;
        console.log(`Filtering schedules for student section: ${this.user.year_section}`);
      }
      
      // For faculty/staff, add instructor filtering
      if (this.user.classification === 'Faculty/Staff') {
        params.instructor_name = this.user.name;
        console.log(`Filtering schedules for instructor: ${this.user.name}`);
      }
      ...
      if (response.data && Array.isArray(response.data)) {
        // Filter schedules to only show approved ones
        let newSchedules = response.data.filter(schedule => {
          // Only include approved schedules
          return schedule.status === 'approved';
        });
        
        // Apply additional filtering for students
        if (this.user.classification === 'Student' && this.user.year_section) {
          newSchedules = newSchedules.filter(schedule => {
            // For students, only show schedules for their section
            return schedule.section === this.user.year_section;
          });
          
          console.log(`Found ${newSchedules.length} schedules for section ${this.user.year_section}`);
        }
        
        // Apply additional filtering for faculty/staff
        if (this.user.classification === 'Faculty/Staff') {
          newSchedules = newSchedules.filter(schedule => {
            // For faculty/staff, only show schedules where they are the instructor
            return schedule.instructor_name === this.user.name;
          });
          
          console.log(`Found ${newSchedules.length} schedules for instructor ${this.user.name}`);
        }
        ...
        if (this.hasSchedulesChanged(this.schedules, processedSchedules)) {
          ...
          console.log(`Updated ${this.schedules.length} schedules for lab ${this.selectedLab}`);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (response.data) {
          ...
          // Important: For students, make sure we get their section
          if (response.data.role === 'Student') {
            this.user.year_section = response.data.year_section || '';
            console.log(`Student section loaded: ${this.user.year_section}`);
          }
  ...
  downloadSchedule() {
    ...
    // Add Year & Section info for students
    let tableTop = margin + 68;
    if (this.user.classification === 'Student') {
      pdf.setFont(undefined, 'bold');
      pdf.text('Year & Section', margin, margin + 66);
      pdf.text(':', margin + 30, margin + 66);
      pdf.setFont(undefined, 'normal');
      pdf.text(this.user.year_section || 'Not specified', margin + 35, margin + 66);
      
      // Adjust the next line position
      pdf.line(margin, margin + 68, pageWidth - margin, margin + 68);
      tableTop = margin + 74;
    } else {
    ...
    // Course Code, Description, Lec/Lab, Schedule, Room, Section/Instructor (%)
    ...
    // Change the last column header based on user classification
    const lastColumnHeader = this.user.classification === 'Student' ? 'Instructor' : 'Section';
    const headers = ['Course Code', 'Description', 'Lec/Lab', 'Schedule', 'Room', lastColumnHeader];
    ...
    processedSchedules.forEach(schedule => {
      ...
      // Set the last column value based on user classification
      const lastColumnValue = this.user.classification === 'Student' ? (schedule.instructor_name || '') : (schedule.section || '');
    ...
    rows.forEach((row, rowIndex) => {
      ...
      // Center the Section/Instructor text (based on user classification)
      const lastColText = trimText(row.lastColumn, maxCharsByCol[5]);
      const lastColWidth = pdf.getStringUnitWidth(lastColText) * pdf.getFontSize() / pdf.internal.scaleFactor;
      const lastColCenter = colPositions[5] + (colWidths[5] / 2);
      pdf.text(lastColText, lastColCenter, currentY + 5, { align: 'center' });
