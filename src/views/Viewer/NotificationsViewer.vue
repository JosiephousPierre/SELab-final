...
export default {
  ...
  methods: {
    checkAuth() {
      // Check if user is authenticated and has a valid viewer role
      ...
      try {
        ...
        // Verify the user has a viewer role (Student, Faculty/Staff, or Dean)
        const validRoles = ['Faculty/Staff', 'Student', 'Dean'];
        if (!validRoles.includes(userData.role)) {
          console.error('User does not have a valid viewer role');
    ...
    async fetchNotifications() {
      this.loading = true;
      try {
        this.notifications = await NotificationService.getNotifications(this.selectedFilter, this.sortBy);
      } catch (error) {
    ...
    async clearNotifications() {
      try {
        ...
        if (confirm('Are you sure you want to clear all notifications? This cannot be undone.')) {
          // Clear notifications in the database (permanently)
